...
async function loadDashboard() {
    ...
    try {
        ...
        const wRes = await apiFetch(`/api/accounting/dashboard/widgets?company_id=${currentCompanyId}`);
        ...
        // Tax Estimate
        const taxRes = await apiFetch(`/api/accounting/tax/estimate?year=${year}&company_id=${currentCompanyId}`);
        ...
        const calRes = await apiFetch(`/api/accounting/tax/compliance-calendar?year=${year}&company_id=${currentCompanyId}`);
...
async function loadTrialBalanceDashboard() {
    ...
    try {
        ...
        const res = await apiFetch(`/api/accounting/trial-balance?company_id=${currentCompanyId}&start=2000-01-01`);
...
async function loadSubscriptions() {
    ...
    try {
        const res = await apiFetch('/api/admin/client-services');
...
async function loadExpenses() {
    try {
        const res = await apiFetch(`/api/accounting/expenses?company_id=${currentCompanyId}`);
...
async function loadCoADropdown() {
    try {
        const res = await apiFetch(`/api/accounting/coa?company_id=${currentCompanyId}`);
        const accounts = await res.json();
...
function initEditExpenseForm() {
    document.getElementById('formEditExpense').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const res = await apiFetch(`/api/accounting/expenses/${expenseId}?company_id=${currentCompanyId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
...
function initJournalForm() {
    document.getElementById('formJournal').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const res = await apiFetch(`/api/accounting/journal?company_id=${currentCompanyId}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
window.accountingJS = {
    ...
    openExpenseModal: async () => {
        ...
        const res = await apiFetch(`/api/accounting/settings?company_id=${currentCompanyId}`);
    ...
    deleteExpense: async (id) => {
        ...
        try {
            const res = await apiFetch(`/api/accounting/expenses/${id}?company_id=${currentCompanyId}`, {
                method: 'DELETE'
            });
            if (!res.ok) throw new Error(await res.text());
    ...
    viewReport: async (reportType) => {
        try {
            ...
            if (reportType === 'pnl') {
                ...
                const d = await apiFetch(`/api/accounting/reports/balance-sheet?company_id=${currentCompanyId}&asOf=${asOf}`).then(r => r.json());
                if (d.error) throw new Error(d.error);
                ...
                const accounts = await apiFetch(`/api/accounting/trial-balance?company_id=${currentCompanyId}&start=2000-01-01`).then(r => r.json());
    ...
    confirmBulkImport: async () => {
        ...
        try {
            ...
            const res = await apiFetch(`/api/accounting/bulk-import/${batchId}/confirm?company_id=${currentCompanyId}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
...
async function initBulkImportForm() {
    document.getElementById('formBulkImportParse').addEventListener('submit', async (e) => {
        ...
        try {
            const res = await apiFetch(`/api/accounting/bulk-import/parse?company_id=${currentCompanyId}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            const coaRes = await apiFetch(`/api/accounting/coa?company_id=${currentCompanyId}`);
...
async function loadAssets() {
    try {
        const res = await apiFetch(`/api/accounting/assets?company_id=${currentCompanyId}`);
        const data = await res.json();
...
function initAssetForm() {
    ...
    document.getElementById('btnRunDepreciation').addEventListener('click', async () => {
        ...
        try {
            const year = new Date().getFullYear();
            const res = await apiFetch(`/api/accounting/depreciation/post/${year}?company_id=${currentCompanyId}`, { method: 'POST' });
            if (!res.ok) throw new Error(await res.text());
...
async function loadGCTTracker() {
    try {
        const res = await apiFetch(`/api/accounting/tax/gct-status?company_id=${currentCompanyId}`);
        const data = await res.json();
        ...
        const setRes = await apiFetch(`/api/accounting/settings?company_id=${currentCompanyId}`);
